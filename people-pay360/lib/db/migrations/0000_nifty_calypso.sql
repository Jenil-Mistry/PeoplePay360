...
CREATE TABLE "salary_rules" (
	...
	"formula" text,
	"is_active" boolean DEFAULT true NOT NULL,
	CONSTRAINT "rule_computation_check" CHECK (
    ("salary_rules"."computation_type" = 'FIXED' AND "salary_rules"."amount" IS NOT NULL) OR
    ("salary_rules"."computation_type" = 'PERCENTAGE' AND "salary_rules"."percentage" IS NOT NULL AND "salary_rules"."base_code" IS NOT NULL) OR
    ("salary_rules"."computation_type" = 'FORMULA' AND "salary_rules"."formula" IS NOT NULL)
  )
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX "one_attendance_per_employee_per_day" ON "attendance" USING btree ("employee_id","date");--> statement-breakpoint
...
CREATE UNIQUE INDEX "unique_rule_code_per_structure" ON "salary_rules" USING btree ("structure_id","code");--> statement-breakpoint
CREATE INDEX "time_off_allocations_emp_type_idx" ON "time_off_allocations" USING btree ("employee_id","time_off_type_id");--> statement-breakpoint
CREATE INDEX "time_off_requests_emp_status_idx" ON "time_off_requests" USING btree ("employee_id","status");